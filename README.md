...
graph TB
    subgraph Client["🖥️ Client Browser"]
        ...
        BSS["Browser SpeechSynthesis<br/>(English TTS)"]
    end

    subgraph Flask["⚡ Flask Application (app.py)"]
        direction TB
        subgraph Blueprints["Blueprints"]
            MAIN["main_bp<br/>(/, /chat, /documents)"]
            AUTH["auth_bp<br/>(/auth/*)"]
            CHAT["chat_bp<br/>(/chat/*)"]
            DOC["document_bp<br/>(/document/*)"]
        end
        subgraph Routes["Utility Routes"]
            TRANSLATE["/translate"]
            TTS_R["/tts/generate"]
            VTT["/voice-to-text"]
            HEALTH["/health"]
    ...
    subgraph Services["🔧 Service Layer"]
        LLM["LLMService<br/>(Streaming Response)"]
        DOCS["DocumentService<br/>(Parse & Validate)"]
        TTSS["TTSService<br/>(Audio Generation)"]
        PREC["PrecedentService<br/>(Case Search)"]
        TRANS["TranslationService<br/>(Multilingual)"]
        BG["BackgroundProcessor<br/>(Daemon Thread)"]
    end

    subgraph External["🌐 External Services"]
        OLLAMA["Ollama API<br/>(localhost:11434)"]
        GTTS["Google TTS<br/>(gTTS)"]
        MYMEM["MyMemory API<br/>(Translation)"]
        GSPEECH["Google Speech API<br/>(Recognition)"]
    end

    subgraph Data["💾 Data Layer"]
        DB["SQLite Database<br/>(legal_ai.db)"]
        JSON["precedents.json<br/>(20 Landmark Cases)"]
        FILES["File Storage<br/>(static/uploads/)"]
        AUDIO["Audio Cache<br/>(static/audio/)"]
    end
    ...
    style Flask fill:#1e293b,stroke:#10b981,color:#e2e8f0
    style Services fill:#1e293b,stroke:#f59e0b,color:#e2e8f0
...
sequenceDiagram
    ...
    F-->>U: 200 {stream_url: "/chat/stream/<id>"}

    U->>F: GET /chat/stream/<id> (EventSource)
    F->>D: Load last 10 messages (context)
    ...
    loop Token by Token
        L-->>F: Token chunk
        F-->>U: SSE data: {type: "token", content: "..."}
    ...
    F-->>U: SSE data: {type: "complete", audio_file: "..."}
```
...
```mermaid
flowchart LR
    A["📁 File Upload<br/>(PDF/DOCX/TXT)"] --> B["📝 Text Extraction<br/>(PyPDF2/python-docx)"]
    B --> C{"📏 Content<br/>Size?"}
    C -->|"< 5000 chars"| D["⚡ Immediate Analysis<br/>(LLM Summary + Type)"]
    C -->|"≥ 5000 chars"| E["⏳ Background Queue<br/>(BackgroundProcessor)"]
    E -->|"Every 10s"| F["🤖 Async Analysis<br/>(Daemon Thread)"]
    D --> G["✅ Processed Document<br/>(Summary + Metadata)"]
    F --> G

    B --> H["🔍 Legal Validation<br/>(Keyword Check ≥ 3/16)"]
    H --> I{"Valid?"}
    I -->|Yes| C
    I -->|"No (Warning)"| C
...
| `POST` | `/tts/generate` | ✅ | `{text, ttsLanguage\|language}` | Generate TTS audio (MP3) |
